...
public class BookService {
   ...
   public SuccessResponseDto<List<Book>> getBookByName(ReqFindBookDTO reqFindBookDTO) throws NotFoundException {
       ...
       List<Book> foundBooks = bookRepository.findBookByName(new Book(0, reqFindBookDTO.getBookName(),0, 0)).orElseThrow(() -> new NotFoundException("도서정보가 존재하지 않습니다."));
   ...
   public SuccessResponseDto<Optional<Book>> addBook(ReqAddBookDTO reqAddBookDTO) {
     return new SuccessResponseDto<>(
             bookRepository.addBook(new Book(0, reqAddBookDTO.getBookName(), reqAddBookDTO.getAuthorId(), reqAddBookDTO.getPublisherId()))
     );
   }
